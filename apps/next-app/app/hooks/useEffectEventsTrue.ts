		...
		Effect.mapError(
			(error) =>
				new ValidationError({
					...
					issues: [String(error)],
					eventId:
						typeof rawEvent === 'object' && rawEvent !== null && '_id' in rawEvent
							? String(rawEvent._id)
							: undefined,
				})
		)
...
export function useTrueEffectEvents(userId: string) {
	...
	const validatedEvents =
		rawEvents?.map((rawEvent: Doc<'events'> & { timeslots?: unknown }) => {
...
export function useTrueEffectEvent(eventId: string, userId: string) {
	...
	if (rawEvent) {
		...
		const validationPipeline = pipe(
			normalizeEventEffect(rawEvent),
			Effect.tap((event) => Effect.log(`Successfully validated event: ${event._id}`)),
			Effect.catchAll((error) => {
