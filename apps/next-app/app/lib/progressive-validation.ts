...
export const venueAddressValidator = (address: string): ValidationResult => {
	...
	const hasCommonAddressWords =
		/\b(street|st|avenue|ave|road|rd|lane|ln|boulevard|blvd|drive|dr|court|ct|place|pl|way|circle|cir|동|구|시|로|가)\b/i.test(
			address
		);
...
export const guestListDeadlineValidator = (
	deadline: string,
	context?: { eventDate?: string }
): ValidationResult => {
	if (!deadline) {
...
export const promoDeadlineValidator = (
	deadline: string,
	context?: { eventDate?: string; guestDeadline?: string }
): ValidationResult => {
	if (!deadline) {
...
export function useEventFormValidation(initialData: Partial<EventFormData>) {
	...
	const date = useProgressiveValidation(initialData.date || '', [eventDateValidator], 300);
