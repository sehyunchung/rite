...
const isCSVData = (
	data: ExportData
): data is { content: string; filename: string; mimeType: string } => {
...
const processExportEffect = (exportData: ExportData, format: string) =>
	Effect.gen(function* () {
		...
		return yield* Effect.fail(
			new ExportProcessingError(`Invalid data format for ${format} export`, format)
		);
	}).pipe(
...
export function ExportGuestList({ eventId, userId }: ExportGuestListProps) {
	...
	const csvData = useQuery(api.exports.exportGuestListCSV, {
		eventId,
		userId,
	});
	...
	const handleExport = React.useCallback(
		async (format: 'csv' | 'excel' | 'pdf' | 'google_sheets') => {
			...
			try {
				...
				if (result.success) {
					if (format === 'google_sheets') {
						toast.success(
							t('success.googleSheets') ||
								'Data copied to clipboard. Google Sheets opened in new tab.'
						);
					} else {
						toast.success(
				...
				<CardContent>
					<p className="text-muted-foreground">
						{t('errors.accessDenied') ||
							'Unable to access export data. Please check your permissions.'}
					</p>
				</CardContent>
			</Card>
		);
			...
			<CardContent>
				<div className="space-y-4">
					...
					<div className="flex items-center space-x-2 text-sm text-muted-foreground">
						...
						<span>
							{t('guestCount', { count: guestCount }) ||
								`${guestCount} guests available for export`}
						</span>
					</div>
					...
					<div className="text-xs text-muted-foreground">
						{t('description') ||
							'Export guest list data in various formats for venue management and reporting.'}
					</div>
				</div>
			</CardContent>
		</Card>
