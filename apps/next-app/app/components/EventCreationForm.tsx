...
export function EventCreationForm({ onEventCreated }: EventCreationFormProps) {
	...
	const addTimeslot = () => {
		if (timeslots.length >= MAX_TIMESLOTS_PER_EVENT) {
			setErrors({
				...errors,
				maxTimeslots: `Maximum ${MAX_TIMESLOTS_PER_EVENT} timeslots allowed per event`,
			});
			return;
		}
	...
	const validateForm = (): boolean => {
		...
		const deadlineOrderError = validateDeadlineOrder(
			formData.deadlines.guestList,
			formData.deadlines.promoMaterials
		);
								...
								<div className="space-y-2">
									...
									<Input
										...
										className={
											...
											errors[`timeslot-${slot.id}-duration`]
												? 'border-red-500'
												: ''
										}
										required
