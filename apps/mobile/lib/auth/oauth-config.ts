...
const getGoogleIOSScheme = (): string => {
	...
	if (!iosClientId) {
		throw new Error(
			'EXPO_PUBLIC_GOOGLE_OAUTH_CLIENT_ID_IOS environment variable is required for iOS OAuth'
		);
	}
	...
	if (parts.length < 2) {
		throw new Error(
			`Invalid iOS client ID format. Expected format: [numbers]-[string].apps.googleusercontent.com, got: ${iosClientId}`
		);
	}
