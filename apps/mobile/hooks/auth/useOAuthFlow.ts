 ...
 */
export const useOAuthFlow = (convex: ConvexReactClient, onAuthSuccess: (user: User) => void) => {
	...
	const signIn = useCallback(async () => {
		if (!hasGoogleConfig() || !request) {
			throw new AuthError('OAuth not configured or request not ready', 'OAUTH_NOT_READY');
		}
		...
		try {
			...
			throw new AuthError('OAuth flow failed', 'OAUTH_FLOW_ERROR', error);
		}
	}, [request, promptAsync]);
