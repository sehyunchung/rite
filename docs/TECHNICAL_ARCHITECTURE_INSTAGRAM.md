...
- Obfuscate access tokens using the platform's basic data obfuscation system
...
export const dailyUsageSummary = cronJobs.interval(
  ...
  async (ctx) => {
    ...
    const publishedToday = await ctx.db
      .query('scheduled_posts')
      .filter((q) =>
        q.and(q.gte(q.field('publishedAt'), today), q.eq(q.field('status'), 'published'))
      )
      .collect();
