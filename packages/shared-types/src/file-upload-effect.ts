 ...
 */
export const validateFileEffect = (file: FileToUpload) =>
	Effect.gen(function* () {
		// Validate size
 ...
 */
export const createProgressTracker = () => Ref.make<UploadProgress | null>(null);
...
export const uploadFileToStorage = (
	...
	Effect.async<UploadResult, UploadNetworkError>((resume) => {
		...
		if (typeof XMLHttpRequest === 'undefined') {
			...
			formData.append('file', file.blob as Blob, file.name);
...
export const uploadFile = (
	...
	pipe(
		...
		// Upload file
		Effect.flatMap(({ uploadUrl, storageId }) =>
			uploadFileToStorage(file, uploadUrl, options?.onProgress).pipe(
				Effect.map((result) => ({
					...result,
 ...
 */
export const createFileStream = (file: Blob, _chunkSize = 1024 * 1024) =>
	Stream.async<Uint8Array, Error>((emit) => {
 ...
 */
export const formatUploadError = (error: FileUploadError): string => {
	switch (error._tag) {
		case 'FileTooLargeError':
			return `File "${error.fileName}" is too large (${formatFileSize(
				error.size
			)}). Maximum size is ${formatFileSize(error.maxSize)}.`;
		case 'InvalidFileTypeError':
			return `File "${error.fileName}" has invalid type "${
				error.type
