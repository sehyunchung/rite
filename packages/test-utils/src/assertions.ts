...
export const assertNotification = (
	message: string,
...
export const assertButtonState = (
	...
	if (state.disabled !== undefined) {
		state.disabled ? expect(button).toBeDisabled() : expect(button).toBeEnabled();
	}
...
export const assertApiCalled = (
	...
	const calls = fetchMock.mock.calls.filter((call: any[]) => call[0].includes(endpoint));
...
export const assertAria = (element: HTMLElement, attributes: Record<string, string>) => {
...
export const assertElementCount = (container: HTMLElement, selector: string, count: number) => {
...
export const assertConvexQuery = (mockQuery: any, queryName: string, expectedArgs?: any) => {
