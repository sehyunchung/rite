		...
		try {
			...
			for (let i = 0; i < timeslots.length; i++) {
				...
				try {
					...
					throw new Error(
						`Failed to create timeslot ${i + 1}: ${timeslotError instanceof Error ? timeslotError.message : 'Unknown error'}`
					);
				}
			}
			...
			if (timeslotResults.length !== timeslots.length) {
				throw new Error(
					`Expected ${timeslots.length} timeslots but only created ${timeslotResults.length}`
				);
			}
...
export const deleteEvent = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (submissions.length > 0) {
			throw new Error(
				'Cannot delete event with existing submissions. Consider cancelling instead.'
			);
		}
