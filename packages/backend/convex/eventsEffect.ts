...
// Error types
class TimeslotValidationError extends S.TaggedError<TimeslotValidationError>()("TimeslotValidationError", {
  message: S.String,
  index: S.Number
}) {}
...
const validateTimeslots = (timeslots: unknown[]) =>
  Effect.gen(function* (_) {
    if (!timeslots || timeslots.length === 0) {
      return yield* _(Effect.fail(new TimeslotValidationError({
    ...
    const validated = yield* _(Effect.forEach(timeslots, (slot, index) =>
      pipe(
        S.decodeUnknown(TimeslotInputSchema)(slot),
        Effect.mapError(() => new TimeslotValidationError({
...
const updateEventCapabilities = (ctx: any, eventId: Id<"events">) =>
  Effect.gen(function* (_) {
    ...
    const timeslots = yield* _(Effect.tryPromise({
      try: () => ctx.db
        .query("timeslots")
...
const createEventEffect = (ctx: any, args: any) =>
  Effect.gen(function* (_) {
    ...
    const timeslotResults = yield* _(pipe(
      Effect.forEach(
        validatedTimeslots,
      ...
      Effect.tap((results) => 
        Effect.log(`Created ${results.length} timeslots`)
      ),
