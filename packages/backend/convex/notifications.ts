...
export const triggerDJStatusNotification = action({
	args: {
		submissionId: v.id('submissions'),
		status: v.union(v.literal('received'), v.literal('accepted'), v.literal('rejected')),
...
export const scheduleEventReminders = action({
	...
	handler: async (
		...
		success: boolean;
		scheduledReminders: Array<{ days: number; scheduledFor: string }>;
	}> => {
		const event = await ctx.runQuery(api.events.getEventById, {
...
export const sendEventReminder = internalAction({
	...
	handler: async (
		...
		success: boolean;
		emailsSent: Array<{ djName: string; email: string; emailId: any }>;
	}> => {
		const event = await ctx.runQuery(api.events.getEventById, {
			eventId: args.eventId,
		});
