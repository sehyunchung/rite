...
export function AlertDialogOverlay({ className }: AlertDialogOverlayProps) {
	return <View className={cn('absolute inset-0 bg-black/50', className)} />;
}
...
export function AlertDialogFooter({ children, className }: AlertDialogFooterProps) {
	return <View className={cn('flex-row justify-end gap-2 mt-4', className)}>{children}</View>;
}
...
export function AlertDialogAction({ children, className, onPress }: AlertDialogActionProps) {
	...
	return (
		<Pressable
			...
			className={cn('bg-gray-900 dark:bg-white px-4 py-2 rounded-md', className)}
		>
